...
export async function GET(req: Request) {
  try {
    ...
    console.error("[lesson-progress GET]", e);
...
export async function POST(req: Request) {
  try {
    ...
    const courseId =
      typeof body === "object" && body !== null && "courseId" in body ? String((body as { courseId: unknown }).courseId) : "";
    const lessonId =
      typeof body === "object" && body !== null && "lessonId" in body ? String((body as { lessonId: unknown }).lessonId) : "";
    ...
    console.error("[lesson-progress POST]", e);
    return NextResponse.json({ error: "Server error" }, { status: 500 });
  }
}
